...
class PositionSkill(Base):
    ...
    # __table_args__ = (UniqueConstraint('position_id', 'skill_id', name="position_skill_uc"),)
...
class CompanyPerk(Base):
    ...
    __table_args__ = (UniqueConstraint('company_id', 'perk_id', name="company_perks_uc"),)

    id_company_perk = Column(Integer, primary_key=True, autoincrement=True)
    company_id = Column(Integer, ForeignKey('company.id_company'))
...
class Company(Base):
    ...
    avg_reputation = Column(Float(2), nullable=False)
    total_ratings = Column(Integer, nullable=False)
    death_line = Column(DateTime, nullable=True)
    ceo_score = Column(Float(2), nullable=False)
...
class CompanyLocation(Base):
    ...
    __table_args__ = (UniqueConstraint('company_id', 'location_id', name="company_location_uc"),)
